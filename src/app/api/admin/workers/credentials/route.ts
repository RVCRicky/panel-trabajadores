...
export async function POST(req: Request) {
  try {
    ...
    if (!workerId) return NextResponse.json({ ok: false, error: "Missing workerId" }, { status: 400 });
    if (!email && !password) return NextResponse.json({ ok: false, error: "Provide email and/or password" }, { status: 400 });
