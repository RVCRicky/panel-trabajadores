...
type Body = {
  invoiceId: string;
  label: string;
  amountEur: number; // positivo = extra, negativo = sanción
};
...
export async function POST(req: Request) {
  try {
    ...
    if (!invoiceId || !label || !Number.isFinite(amountEur)) {
    ...
    // insert manual line
    const { error: insErr } = await supabaseAdmin.from("worker_invoice_lines").insert({
    ...
    // recalc (SQL function)
    const { error: rpcErr } = await supabaseAdmin.rpc("recalc_invoice", { p_invoice_id: invoiceId });
